...
const Input: React.FC<InputProps> = ({ label, status, message, type, isRequired, id }) => {
    const statusType = [
        {
            name: "default",
            value: {
...

        {
            ...
            value: {
                ...
                messageColor: "mt-1 text-left text-xs font-normal text-green-600 dark:text-green-500",
            },
        },
        {
            name: "error",
            value: {
                ...
                messageColor: "mt-1 text-left text-xs font-normal text-[#FF1717] dark:text-[#FF1717]",
            },
        },
    ];
            ...
            {currentStatus?.value && (
                <>
                    <div className="relative">
                        <input type={type} id={id} className={currentStatus.value.inputClass} placeholder="" />
                        <label htmlFor={id} className={currentStatus.value.labelClass}>
                            {label} {isRequired && <span className="text-[#FF3C3C]">*</span>}
                        </label>
                    </div>
                    {currentStatus.name !== "default" && <p className={currentStatus.value.messageColor}>{message}</p>}
                </>
            )}
        </>
    );
